// V7-fix2 — Amwaj Apartments
// Все фото берём только из /img/<ID>/1.jpeg … 12.jpeg
...
function imgMainPath(id){ return `img/${id}/1.jpeg`; }
function buildGalleryList(id,count){ return Array.from({length:count},(_,i)=>`img/${id}/${i+1}.jpeg`); }
...
function renderCards(){
  ...
  APARTMENTS.forEach(a => {
    ...
    const main = hasPhotos ? imgMainPath(a.id) : placeholder(`${a.corp}-${a.id}`);
    const card = document.createElement('article');
    card.className = 'card';
    card.innerHTML = `
      <div class="photo"><img alt="${a.title}" src="${main}" onerror="this.src='${placeholder(a.id)}'"/></div>
      <div class="card-body">
        <h3>${a.title}</h3>
        ...
        <div class="actions">
          ...
          ${hasPhotos ? `<button class="btn ghost" data-open="${a.id}">Галерея</button>` : `<span class="muted">Фото будут позже</span>`}
        </div>
      </div>`;
...
let galleryList=[], galleryIdx=0, gTitle='';
function openGallery(id){
  const a = APARTMENTS.find(x=>x.id===id); if(!a) return;
  galleryList = buildGalleryList(id,a.photosCount);
  if(!galleryList.length){galleryList=[placeholder(id)];}
  galleryIdx=0;
  gTitle=`Корпус ${a.corp}, апартамент ${id}`;
  document.getElementById('gTitle').textContent=`${gTitle} — 1/${galleryList.length}`;
...
function navGallery(step){
  ...
  document.getElementById('gTitle').textContent=`${gTitle} — ${galleryIdx+1}/${galleryList.length}`;
}
...
document.addEventListener('DOMContentLoaded',()=>{
  renderCards();
  document.getElementById('prev').onclick=()=>navGallery(-1);
  document.getElementById('next').onclick=()=>navGallery(1);
