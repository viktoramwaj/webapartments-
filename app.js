(() => {
  ...
  function updateGallery(){const p=photosFor(currId);const total=p.length||1;const showAt=(i)=>{idx=i%total;const src0=p[idx];gTitle.textContent=`Апартамент ${currId} — ${idx+1}/${total}`;let triedAlt=false;gImg.onerror=()=>{if(!triedAlt){triedAlt=true;if(src0.endsWith('.jpeg'))gImg.src=src0.slice(0,-5)+'.jpg';else if(src0.endsWith('.jpg'))gImg.src=src0.slice(0,-4)+'.jpeg';else skip();}else{skip();}};gImg.onload=()=>{gImg.onerror=null;};gImg.src=src0;function skip(){gImg.onerror=null;if(total<=1)return;showAt((idx+1)%total);} };showAt(idx);}
